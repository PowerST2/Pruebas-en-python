...
% Entorno para mantener preguntas completas en una página
\\RequirePackage{needspace}

% Comando para pregunta completa que no se divide entre páginas
\\newcommand{\\preguntacompleta}[3]{
\\needspace{8\\baselineskip}% Requiere al menos 8 líneas de espacio
\\begin{samepage}
\\item \\import{#1}{#2}
\\rpt{#3}
\\end{samepage}
...
% Comando para títulos de sección que siempre aparecen en la parte superior de columna
\\newcommand{\\tituloseccion}[1]{
% Si estamos en la mitad inferior de una columna, forzar salto a la siguiente columna
% En formato twocolumn, cada columna tiene aproximadamente 0.45\\textheight
% Pedimos 0.25\\textheight para asegurar que esté en la parte superior
...
def exam():
	if len(periodo.get()) != 0:
		...
		for i in range(num_temas):
			# abre el tema
			prueba=open('Prueba%s/Prueba%s.tex' % (temas[i],temas[i]),mode='a',encoding='utf-8')
			prueba.write(r'\tituloseccion{%s}' % 'FÍSICA' +'\n'+r'\begin{enumerate}[1.]'+'\n')
			for j in range(len(preg[i])):
				clave=open('%s/%s/visor.tex' % ('fisica',preguntas[preg[i][j]]),mode='r',encoding='utf-8')
				rpt=clave.readlines()
				prueba.write(r'\preguntacompleta{../%s/%s/}{%s}{%s}' % ('fisica',preguntas[preg[i][j]],preguntas[preg[i][j]],letra(rpt[9][12:20])) +'\n')
				clave.close
		...
		for i in range(num_temas):
			# abre el tema
			prueba=open('Prueba%s/Prueba%s.tex' % (temas[i],temas[i]),mode='a',encoding='utf-8')
			prueba.write(r'\tituloseccion{%s}' % 'QUÍMICA' +'\n'+r'\begin{enumerate}[1.]'+'\n'+
						r'\setcounter{enumi}{\value{nx}}'+'\n')
			for j in range(len(preg[i])):
				clave=open('%s/%s/visor.tex' % ('quimica',preguntas[preg[i][j]]),mode='r',encoding='utf-8')
				rpt=clave.readlines()
				prueba.write(r'\preguntacompleta{../%s/%s/}{%s}{%s}' % ('quimica',preguntas[preg[i][j]],preguntas[preg[i][j]],letra(rpt[9][12:20])) +'\n')
				clave.close
			prueba.write(r'\setcounter{nx}{\value{enumi}}'+'\n'+r'\end{enumerate}'+'\n')
		...
		for i in range(num_temas):
			# abre el tema
			prueba=open('Prueba%s/Prueba%s.tex' % (temas[i],temas[i]),mode='a',encoding='utf-8')
			prueba.write(r'\tituloseccion{%s}' % 'MATEMÁTICA' +'\n')
			...
			for j in range(len(preg[i])):
				...
				rpt=clave.readlines()
				prueba.write(r'\preguntacompleta{../matematica/%s/%s/}{%s}{%s}' % ('aritmetica',preguntas[preg[i][j]],preguntas[preg[i][j]],letra(rpt[9][12:20])) +'\n')
				clave.close
			prueba.write(r'\setcounter{nx}{\value{enumi}}'+'\n'+r'\end{enumerate}'+'\n')
		...
		for i in range(num_temas):
			...
			for j in range(len(preg[i])):
				...
				rpt=clave.readlines()
				prueba.write(r'\preguntacompleta{../matematica/%s/%s/}{%s}{%s}' % ('algebra',preguntas[preg[i][j]],preguntas[preg[i][j]],letra(rpt[9][12:20])) +'\n')
				clave.close
			prueba.write(r'\setcounter{nx}{\value{enumi}}'+'\n'+r'\end{enumerate}'+'\n')
		...
		for i in range(num_temas):
			...
			for j in range(len(preg[i])):
				...
				rpt=clave.readlines()
				prueba.write(r'\preguntacompleta{../matematica/%s/%s/}{%s}{%s}' % ('geometria',preguntas[preg[i][j]],preguntas[preg[i][j]],letra(rpt[9][12:20])) +'\n')
				clave.close
			prueba.write(r'\setcounter{nx}{\value{enumi}}'+'\n'+r'\end{enumerate}'+'\n')			
		...
		for i in range(num_temas):
			...
			for j in range(len(preg[i])):
				...
				rpt=clave.readlines()
				prueba.write(r'\preguntacompleta{../matematica/%s/%s/}{%s}{%s}' % ('trigonometria',preguntas[preg[i][j]],preguntas[preg[i][j]],letra(rpt[9][12:20])) +'\n')
				clave.close
			prueba.write(r'\setcounter{nx}{\value{enumi}}'+'\n'+r'\end{enumerate}'+'\n')			
		...
		for i in range(num_temas):
			# abre el tema
			prueba=open('Prueba%s/Prueba%s.tex' % (temas[i],temas[i]),mode='a',encoding='utf-8')
			prueba.write(r'\tituloseccion{%s}' % 'RAZONAMIENTO MATEMÁTICO' +'\n'+r'\begin{enumerate}[1.]'+'\n'+
						r'\setcounter{enumi}{\value{nx}}'+'\n')
			for j in range(len(preg[i])):
				clave=open('%s/%s/visor.tex' % ('aptitud_academica',preguntas[preg[i][j]]),mode='r',encoding='utf-8')
				rpt=clave.readlines()
				prueba.write(r'\preguntacompleta{../%s/%s/}{%s}{%s}' % ('aptitud_academica',preguntas[preg[i][j]],preguntas[preg[i][j]],letra(rpt[9][12:20])) +'\n')
				clave.close
			prueba.write(r'\setcounter{nx}{\value{enumi}}'+'\n'+r'\end{enumerate}'+'\n') 
		...
		for i in range(num_temas):
			# abre el tema
			prueba=open('Prueba%s/Prueba%s.tex' % (temas[i],temas[i]),mode='a',encoding='utf-8')
			prueba.write(r'\tituloseccion{%s}' % 'HUMANIDADES' +'\n'+r'\begin{enumerate}[1.]'+'\n'+
						r'\setcounter{enumi}{\value{nx}}'+'\n')
			for j in range(len(preg[i])):
				clave=open('%s/%s/visor.tex' % ('humanidades',preguntas[preg[i][j]]),mode='r',encoding='utf-8')
				rpt=clave.readlines()
				prueba.write(r'\preguntacompleta{../%s/%s/}{%s}{%s}' % ('humanidades',preguntas[preg[i][j]],preguntas[preg[i][j]],letra(rpt[9][12:20])) +'\n')
				clave.close
			prueba.write(r'\setcounter{nx}{\value{enumi}}'+'\n'+r'\end{enumerate}'+'\n'+r'\end{document}') 	
